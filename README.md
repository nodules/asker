...
* `stringify` — Used by default. Applies `JSON.stringify` if `Object` was passed to `body` (otherwise `text` encoder will be used). Accepts `Object`.
...
Asker.bodyEncoders.trimText = {     // encoder name is 'trimText'
    ...
    type : '*',                     // any type of `body` options is allowed by this encoder
