 ...
 * @property {String}   [bodyEncoding] Body encoding method = multipart|urlencoded|plain|stringify (default)
 *      if multipart chosen, there are two ways to transfer file content at body:
 *          directly as buffer: { param_name: <Buffer ...> }
 *          with extended info: { param_name: {filename: 'pic.jpg', mime: 'image/jpeg', path: './pic.jpg' or data: <Buffer ...>} }
...
function Request(options, callback) {
    ...
    if (typeof this.options.body !== 'undefined') {
        var boundary = 'AskerBoundary-' + parseInt(Math.random()*1e16) + '-' + +new Date;
        var contentType = {
        ...
        if ( ! contentType[this.options.bodyEncoding]) {
            this.options.bodyEncoding = 'stringify';
        }
        if (this.options.bodyEncoding === 'stringify' && typeof this.options.body !== 'object') {
            this.options.bodyEncoding = 'text';
        }

        switch (this.options.bodyEncoding) {
            case 'urlencoded':
                this.options.body = url.format({query: this.options.body}).substr(1);
                break;
            case 'multipart':
                var CRLF = "\r\n",
                    bufs = [];
                ...
                Object.keys(this.options.body).forEach(function(name) {
                    var headers = [],
                        isFile = typeof this.options.body[name] === 'object',
                        isBuffer = isFile && Buffer.isBuffer(this.options.body[name]);
                    headers.push('--' + boundary);
                    var disposition = 'content-disposition: form-data; name="' + name + '"';
                    if (isFile && !isBuffer && 'filename' in this.options.body[name]) {
                        disposition += '; filename="' + this.options.body[name].filename + '"';
                    }
                    headers.push(disposition);
                    if (isFile) {
                        var headerType = 'content-type: ';
                        headerType += !isBuffer && 'mime' in this.options.body[name] ?
                            this.options.body[name].mime :
                    ...
                    if (isFile) {
                        if (isBuffer) {
                            data = this.options.body[name];
                        } else {
                            if ('path' in this.options.body[name]) {
                                data = fs.readFileSync(this.options.body[name].path);
                            } else if ('data' in this.options.body[name]) {
                                data = this.options.body[name].data;
                            }
                        }
                    } else {
                        data = new Buffer(this.options.body[name]);
                    }
                    bufs.push(data);
                    bufs.push(new Buffer(CRLF));
                }.bind(this));
                bufs.push(new Buffer('--' + boundary + '--'));
                this.options.body = Buffer.concat(bufs);
                console.log(this.options.body.toString('utf8'));
                break;
            case 'stringify':
                this.options.body = JSON.stringify(this.options.body);
                break;
            case 'text':
                this.options.body = String(this.options.body);
                break;
        }
        if ( ! this.options.headers['content-type']) {
            this.options.headers['content-type'] = contentType[this.options.bodyEncoding];
        }
...
Request.createAgent = function(options) {
    ...
    if (Object.prototype.hasOwnProperty.call(agentsPool, options.name)) {
