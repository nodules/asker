 ...
 * @returns {String} Full timer nme, which satisfies the pattern "asker.<timerIdPrefix>.<timerId>"
 */
Request.prototype.buildTimerId = function(timerId) {
    var base = 'asker.';

    if (this.options.timerIdPrefix) {
        base += this.options.timerIdPrefix + '.';
    } else if (this.options.requestId) {
