...
function Request(options, callback) {
    ...
    this.options = assign({}, Request.DEFAULT_OPTIONS, options);
    ...
    if (this.options.query) {
        ...
        assign(parsedPath.query, this.options.query);

        this.options.path = url.format(parsedPath);
    }

    // @todo add user-agent headers
    // if ( ! this.options.headers['user-agent']) {
    //     this.options.headers['user-agent'] = 'asker/1.0';
    // }
...
Request.getAgent = function(request) {
    var agent = request.options.agent;
    ...
    // checks custom agent
    if (typeof agent === 'object' && agent.addRequest && agent.createSocket) {
        return agent;
    }

    var protocol = request.options.protocol.split(':')[0],
    ...
    return assignedAgent || new AdvancedAgent(assign({ persistent : true }, agent));
};
...
Request.prototype.execute = function(callback) {
    ...
    this._tryHttpRequest(assign({}, this.options, { agent : Request.getAgent(this) }));
