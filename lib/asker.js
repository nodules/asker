...
function Request(options, callback) {
    ...
    if (['POST', 'PUT', 'PATCH', 'DELETE'].indexOf(this.options.method) > -1 && typeof this.options.body !== 'undefined') {
...
Request.prototype._tryHttpRequest = function(options) {
    ...
    httpRequest = http.request(options, function(res) {
        var contentLen = parseInt(res.headers['content-length'], 10),
            isContentLength = ! isNaN(contentLen) && contentLen > 0,
            offset = 0,
            error,
            body;

        if (isContentLength) {
            body = new Buffer(contentLen);
            body.fill(0);
        }
        ...
        res.on('data', function(chunk) {
            ...
            if (isContentLength) {
                ...
                body = typeof body !== 'undefined' ? Buffer.concat([body, chunkBuffer]) : chunkBuffer;
            }
        });
        ...
        res.on('end', function() {
            ...
            if (encoding && encoding.toLowerCase().indexOf('gzip') > -1) {
                ...
                self.resolve(res.statusCode, typeof body !== 'undefined' ? body.toString() : null, {}, res.headers);
            }
        });
    });
