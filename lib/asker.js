 ...
 * @param {Object} [meta]
 */
Request.prototype.resolve = function(code, response, meta) {
    this.done(null, {
        data : response,
        statusCode : code,
            ...
            if (encoding && encoding.toLowerCase().indexOf('gzip') > -1) {
                zlib.gunzip(body, function(err, deflatedBody) {
                    error ?
                        ...
                        self.resolve(res.statusCode, deflatedBody.toString(), { gzip : true });
                });
            } else {
                self.resolve(res.statusCode, typeof body !== 'undefined' ? body.toString() : null);
            }
        });
    });
