var http = require('http'),
    ...
    bufferConcat = require('./buffer_concat_polyfill').bufferConcat,
 ...
 * @property {*}        [body] Request body, JSON.stringify applies if Object
...
function Request(options, callback) {
    ...
    if (this.options.url) {
        ...
        if ( ! this.options.port) {
            this.options.port = Request.DEFAULT_OPTIONS.port;
        }
    }
...
Request.createAgent = function(options) {
    ...
    if ( ! agent.options.persistent) {
        agent.on(AdvancedAgent.EVENTS.SOCKET_REMOVED, function() {
            if (( ! this.requests.keys || this.requests.keys().length === 0) &&
                ( ! this.sockets.keys || this.sockets.keys().length === 0)) {
...
Request.prototype._tryHttpRequest = function(options) {
    ...
    httpRequest = http.request(options, /** @param {http.IncomingMessage} res */ function(res) {
        ...
        res.on('end', function() {
            ...
            body = bufferConcat(body, bodyLength);
            ...
            if (encoding && encoding.toLowerCase().indexOf('gzip') > -1) {
                zlib.gunzip(body, function(error, deflatedBody) {
                    if (error) {
                        ...
                        self.resolve(res.statusCode, deflatedBody.toString(), { gzip : true }, res.headers);
                    }
                });
            } else {
                self.resolve(res.statusCode, body.length > 0 ? body.toString() : null, {}, res.headers);
            }
        });
    });
    ...
    httpRequest.break = function(abort, errorCode, errorData) {
        ...
        // @todo looks like always true and implicit now
        if (abort) {
    ...
    }, isNaN(options.queueTimeout) ? options.timeout + self.QUEUE_TIMEOUT_DELTA : options.queueTimeout);
    ...
    httpRequest.on('error', function(error) {
        if ( ! httpRequest.rejected) {
            ...
            httpRequest.break(true, AskerError.CODES.HTTP_CLIENT_REQUEST_ERROR, error);
        }
    });
...
Request.prototype.execute = function() {
    ...
    // @todo move this.agent to this.options.agent in the constructor?
