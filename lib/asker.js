...
function Request(options, callback) {
    ...
    // set status codes filter if passed in the options
    if (typeof this.options.statusFilter === 'function') {
        this._statusCodeFilter = this.options.statusFilter;
    } else {
        this._statusCodeFilter = null;
    }
...
/**
 * Processes status code with internal `statusCodeFilter`.
 * Returns `true` if status code is allowed or `false` if retry should be done.
 *
 ...
 * @returns {Boolean}
 */
Request.prototype.isNetworkError = function(code) {
    if (typeof this._statusCodeFilter === 'function') {
        var result = this._statusCodeFilter(code);
        // @note invert `accept` result for backward compatibility with asker 0.6.x
        return typeof result === 'object' ? (! result.accept) : result;
    }
    return code > 499 || code < 200;
};
...
Request.prototype._tryHttpRequest = function(options) {
    ...
    var httpRequest = requestModule.request(options, /** @param {http.IncomingMessage} res */ function(res) {
        var isNetworkError = self.isNetworkError(res.statusCode);

        // if status code isn't accepted by status filter
        // then abort current request execution and
        // retry request with `UNEXPECTED_STATUS_CODE` error
        if (isNetworkError) {
            httpRequest.break();
    ...
    httpRequest.break = function(errorCode, errorData) {
        ...
        // force agent "freeness" (e.g. release) in Node.js<0.12
        httpRequest.abort();
