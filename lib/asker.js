...
function Request(options, callback) {
    ...
    if (this.options.url) {
        ...
        this.options.protocol = parsedUrl.protocol.split(':')[0];
...
Request.getAgent = function(request) {
    var options = request.options.agent,
        AdvancedAgent = agent.get(options.protocol || 'http'),
        assignedAgent = false;
    ...
    if ( ! options || ! options.name || options.name === 'globalAgent') {
        assignedAgent = options.protocol === 'https' ? https.globalAgent : http.globalAgent;
    } else {
        assignedAgent = AdvancedAgent.get(options.name);
    }
