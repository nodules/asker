 ...
 * @property {Function} [isNetworkError] (code Number) Checks whether the given status code should be treated as network error
 *    By default only codes less than 500 are acceptable.
...
Request.prototype._tryHttpRequest = function(options) {
    ...
    var httpRequest = requestModule.request(options, /** @param {http.IncomingMessage} res */ function(res) {
        ...
        if (error) {
            ...
            return httpRequest.break(error);
        }
     ...
     */
    httpRequest.break = function(retryReason) {
        httpRequest.clearTimeouts();
    ...
    queueTimeout = setTimeout(function() {
        httpRequest.break(AskerError.createError(AskerError.CODES.QUEUE_TIMEOUT));
    ...
    httpRequest.on('socket', function() {
        ...
        if ( ! socketTimeout) {
            socketTimeout = setTimeout(function() {
                httpRequest.break(AskerError.createError(AskerError.CODES.SOCKET_TIMEOUT));
            }, options.timeout);
        }
    });
    ...
    httpRequest.on('error', function(error) {
        if ( ! httpRequest.rejected) {
            ...
            httpRequest.break(AskerError.createError(AskerError.CODES.HTTP_CLIENT_REQUEST_ERROR, error));
        }
    });
